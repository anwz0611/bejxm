...
public class MPChartHelper {
    ...
    public static void setCombineChart(CombinedChart combineChart, final List<String> xAxisValues, List<Float> lineValues, List<Float> barValues, String lineTitle, String barTitle) {
        ...
        combineChart.setNoDataText("暂无数据"); // 没有数据时显示的内容
...
//        xAxis.setLabelCount(xAxisValues.size() + 2);
        ...
        rightAxis.setAxisLineWidth(2);//设置轴线宽度
...
//        combineChart.animateY(5000,Easing.EasingOption.EaseInSine);
    ...
    private static LineData generateLineData(List<Float> lineValues, String lineTitle) {
        ...
        lineData.setValueFormatter(new IValueFormatter() {
            @Override
            public String getFormattedValue(float value, Entry entry, int i, ViewPortHandler viewPortHandler) {
                return StringUtils.double2String(value, 2)+"m3/n";
            }
        });
     ...
     * 生成线图数据
     */
    private static LineData generateLineLiftData(List<Float> lineValues, String lineTitle) {
        ArrayList<Entry> lineEntries = new ArrayList<>();
        ...
        LineDataSet lineDataSet = new LineDataSet(lineEntries, lineTitle);
        lineDataSet.setColor(Color.rgb(159, 143, 186));
        lineDataSet.setLineWidth(2.5f);//设置线的宽度
        lineDataSet.setCircleColor(Color.rgb(244, 219, 100));//设置圆圈的颜色
        lineDataSet.setCircleColorHole(Color.WHITE);//设置圆圈内部洞的颜色
        //lineDataSet.setValueTextColor(Color.rgb(254,116,139));
        ...
        lineData.setValueFormatter(new IValueFormatter() {
            @Override
            public String getFormattedValue(float value, Entry entry, int i, ViewPortHandler viewPortHandler) {
                return StringUtils.double2String(value, 2)+"米";
            }
        });
...
//        barDataSet.setColor(Color.RED);
     ...
     * 设置柱线组合图样式，柱图依赖左侧y轴，线图依赖右侧y轴
     */
    public static void setTwolineChart(LineChart lineChart, final List<String> xAxisValues, List<Float> lineValues, List<Float> lineValues1, String lineTitle, String lineTitle1) {
...
//        xAxis.setLabelCount(xAxisValues.size() + 2);
        ...
        LineChartMarkView markerView=new LineChartMarkView(lineChart.getContext(),xAxis.getValueFormatter());
        ...
        Float yMin = Double.valueOf(Collections.min(lineValues1) * 0.9).floatValue();
        Float yMax = Double.valueOf(Collections.max(lineValues1) * 1.1).floatValue();
        leftAxis.setAxisMaximum(yMax);
        leftAxis.setAxisMinimum(yMin);
        ...
        lineChart.setData(getLineData(lineValues,lineTitle,lineValues1,lineTitle1));
     ...
     * 生成线图数据
     */
    private static LineData getLineData(List<Float> lineValues, String lineTitle,List<Float> lineValues1, String lineTitle1) {
        ArrayList<Entry> lineEntries = new ArrayList<>();
        ...
        for (int i = 0, n = lineValues1.size(); i < n; ++i) {
            lineEntries1.add(new Entry(i, lineValues1.get(i)));
        }
        ...
        LineDataSet lineDataSet1 = new LineDataSet(lineEntries1, lineTitle1);
        lineDataSet1.setColor(Color.rgb(238, 0, 0));
        lineDataSet1.setLineWidth(2.5f);//设置线的宽度
        lineDataSet1.setCircleColor(Color.rgb(238, 0, 35));//设置圆圈的颜色
        lineDataSet1.setCircleColorHole(Color.WHITE);//设置圆圈内部洞的颜色
        //lineDataSet.setValueTextColor(Color.rgb(254,116,139));
        lineDataSet1.setAxisDependency(YAxis.AxisDependency.LEFT);//设置线数据依赖于右侧y轴
